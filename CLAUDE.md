...
# Full workflow
python3 -m spyglass profile --mode cpu --filter epoch-boundary -n my-test

# Individual steps
python3 -m spyglass build --mode cpu
python3 -m spyglass run --mode cpu --filter epoch-boundary -n my-test
python3 -m spyglass analyze ./profiles/my-test/cpu/epoch_boundary
python3 -m spyglass compare ./profiles/baseline/cpu/epoch_boundary ./profiles/opt/cpu/epoch_boundary
...
3. **Output paths are auto-constructed** — `<output_dir>/<nickname_or_commit>/<mode>/<filter>/`. The nickname comes from CLI `--nickname`, config `nickname`, or auto-detected commit hash.
...
```bash
python3 -m spyglass analyze /path/to/dir/containing/perf.data
```
...
```bash
git checkout main
python3 -m spyglass profile --mode cpu --filter epoch-boundary -n main
git checkout my-optimization
python3 -m spyglass profile --mode cpu --filter epoch-boundary -n my-opt
python3 -m spyglass compare ./profiles/main/cpu/epoch_boundary ./profiles/my-opt/cpu/epoch_boundary
```
